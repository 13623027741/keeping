...
// 进度条的颜色
#define kCircleColor ([UIColor colorWithRed:100/255.0 green:98/255.0 blue:163/255.0 alpha:1.0])
...
//            [self cleanLayer];
        ...
        case WSLoadStatusFailed: {
            ...
            [path1 addLineToPoint:CGPointMake(10, 50)];
            
            [self.path appendPath:path];
            [self.path appendPath:path1];
            [self setupAnimationWithPath:self.path.CGPath loadSelector:@selector(successOrFailedAnimation) interval:0.015];
            [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(cleanLayer) userInfo:nil repeats:NO];
        }
            break;
        default:
            break;
